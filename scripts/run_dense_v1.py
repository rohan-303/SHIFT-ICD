...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"n": len(rows), "answerable_n": len(answerable), "noncombination_answerable_n": len(noncomb), "combination_n": len(complex_rows)}
    for k in K_VALUES:
...
def main() -> None:
    ...
    dump_json(OUT / "config.json", {"experiment": "dense_v1", "benchmark_version": "1.0", "models": [spec.__dict__ for spec in SPECS], "top_k": 100, "seed": 20260830, "batch_size": {"transformers": 64, "sentence_transformer": 32}, "selection_criterion": ["noncombination_answerable_Hit@100", "CompleteScenarioRetrieval@100", "Hit@10", "MRR"]})
