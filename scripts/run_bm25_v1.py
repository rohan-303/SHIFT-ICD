...
def text_pair(row: pd.Series, mode: str) -> str:
    long = "" if pd.isna(row.get("target_label")) else str(row.get("target_label"))
    short = "" if pd.isna(row.get("target_short_description")) else str(row.get("target_short_description"))
    if mode == "q1_long_only":
        return long or short
    if short and long and short.casefold() != long.casefold():
        return f"{short} {long}"
    return long or short


def build_corpora(rows: pd.DataFrame, mode: str) -> tuple[dict[str, dict[str, str]], dict[str, Any]]:
    ...
    for direction in DIRECTIONS:
        ...
        for row in frame.sort_values(["target_code", "row_id"]).itertuples(index=False):
            ...
            if code not in docs:
                value = text_pair(pd.Series(row._asdict()), mode)
                docs[code] = value
        token_lengths = [len(tokenize(value)) for value in docs.values()]
        normalized = [" ".join(tokenize(value)) for value in docs.values()]
        duplicate_counts = Counter(normalized)
        profiles[direction] = {
            "target_version": str(frame.target_version.iloc[0]) if len(frame) else None,
            "concept_count": len(docs),
            "descriptions_available": sum(bool(value) for value in docs.values()),
            "descriptions_missing": sum(not value for value in docs.values()),
            "duplicate_normalized_description_groups": sum(v > 1 for v in duplicate_counts.values()),
            "duplicate_normalized_description_concepts": sum(v for v in duplicate_counts.values() if v > 1),
            "average_token_length": statistics.mean(token_lengths) if token_lengths else 0.0,
            "median_token_length": statistics.median(token_lengths) if token_lengths else 0.0,
            "vocabulary_size": len({token for value in docs.values() for token in tokenize(value)}),
            "text_representation": mode,
        }
...
def evaluate(examples: list[BenchmarkExample], corpora: dict[str, dict[str, str]], system: str, params: tuple[float, float], seed: int, ranking_dir: Path | None = None, collect_rows: bool = False) -> tuple[list[dict[str, Any]], dict[str, list[float]], list[dict[str, Any]]]:
    ...
    indexes = {direction: BM25Index.from_documents(corpora[direction], *params) if system.startswith("BM25") else None for direction in DIRECTIONS}
...
def main() -> None:
    ...
    out = ROOT / "artifacts/experiments/bm25_v1"
    ...
    corpora, profiles = build_corpora(normalized, config["primary_text"])
    ...
    for dataset_name, dataset in datasets.items():
        for system, params in [("Random", (1.5, 0.75)), ("Exact Label", (1.5, 0.75)), ("Token Overlap", (1.5, 0.75)), ("BM25 Default", (1.5, 0.75)), ("BM25 Dev-Tuned", frozen)]:
    ...
    reports = ROOT / "reports/tables/bm25_v1"
    overall = []
